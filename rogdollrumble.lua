...
MainFrame.Size = UDim2.new(0.3, 0, 0.3, 0) -- Small size initially
...
HitboxButton.Text = "Toggle Hitbox Enlargement"
...
local function toggleSize()
    if MainFrame.Size == UDim2.new(0.3, 0, 0.3, 0) then
        MainFrame.Size = UDim2.new(0.15, 0, 0.15, 0) -- Make smaller
    else
        MainFrame.Size = UDim2.new(0.3, 0, 0.3, 0) -- Make larger
    end
end
...
-- Function to handle enemy hits for one-hit kill
local function hitEnemy(enemy)
    if oneHitKillActive and enemy:FindFirstChild("Humanoid") then
        enemy.Humanoid:TakeDamage(enemy.Humanoid.Health) -- Instantly kill the enemy
        print("Enemy has been one-shotted!")
    end
end

-- Connect button clicks to functions
ToggleSizeButton.MouseButton1Click:Connect(toggleSize)
AutoDodgeButton.MouseButton1Click:Connect(toggleAutoDodge)
HitboxButton.MouseButton1Click:Connect(toggleHitbox)
OneHitKillButton.MouseButton1Click:Connect(toggleOneHitKill)
